...
public class SimplePatrol : MonoBehaviour
{
    ...
    void CreateFX_GC_L()
    {
        ...
        // 射线检测地面高度
        RaycastHit hit;
        if (Physics.Raycast(footPosition + Vector3.up * 1.0f, Vector3.down, out hit, Mathf.Infinity, groundLayer))
        {
            footPosition.y = hit.point.y;  // 将裂痕生成在地面上
        }

        Vector3 offset = new Vector3(0, 0.1f, 0);
        GameObject fx = Instantiate(Resources.Load<GameObject>("Outside_Crack"), footPosition + offset, Quaternion.identity);
